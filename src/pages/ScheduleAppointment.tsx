...
import { ArrowLeft, ArrowRight, CheckCircle2, ChevronLeft, ChevronRight, MessageSquare, Mail, CreditCard, X } from 'lucide-react'
...
export default function ScheduleAppointment() {
  ...
  // Payment channel preference modal states
  ...
  const handleFinalBooking = async () => {
    ...
    const consultTitle = selectedConsult?.title || 'Physiotherapy Consultation'
    const consultPrice = selectedConsult?.price || ''

    const message = `*New Appointment Request*%0A%0A` +
      `*Name:* ${name}%0A` +
      `*Phone:* ${phone}%0A` +
      `*Consultation:* ${consultTitle} (${consultPrice})%0A` +
      `*Date:* ${dateStr}%0A` +
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-24">
          ...
          >
            ...
            <h2 className="font-display font-bold text-heading text-2xl mb-2">Request Received!</h2>
            ...
            <p className="text-body text-sm mb-8">
              Your appointment details and payment link have been dispatched via <strong className="text-heading capitalize">{preferredChannel}</strong> to <strong className="text-heading">{phone}</strong>.
            </p>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Confirmation & Payment Mode Modal */}
      <AnimatePresence>
        {showPreferenceModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-card relative border border-gray-100"
            >
              <button 
              ...
              <h3 className="font-display font-bold text-xl text-heading mb-1">Payment & Confirmation Channel</h3>
              <p className="text-body text-xs mb-6">
                Choose how you want to receive your confirmation text and payment instructions:
              </p>
              ...
              <div className="space-y-3 mb-6">
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-sm text-heading">WhatsApp Message</p>
                    <p className="text-xs text-body">Receive instant payment link & text via WhatsApp</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-sm text-heading">Email Address</p>
                    <p className="text-xs text-body">Receive payment details & appointment receipt via Email</p>
                  </div>
                </div>
              </div>
              ...
              >
                <CreditCard size={18} />
                Proceed to Receive Payment Link
              </button>
            </motion.div>
          </div>
        )}
